...
class EventsController {
    ...
    static createEvent = async (req, res, next) => {
        try {
            ...
            const schema = Joi.object({
                start_date: Joi.date().default(() => moment().format('YYYY-MM-DD HH:mm')).required(),
                end_date: Joi.date().default(() => moment().format('YYYY-MM-DD HH:mm')).required(),
            ...
            if (files.cover_image[0]) {
                const original = path.join('/uploads/images', files.cover_image[0].filename).replace(/\\/g, '/');
                const thumb = path.join('/uploads/images', `thumb-${files.cover_image[0].filename}`).replace(/\\/g, '/');
                await Promise.all([
            ...
            if (!_.isEmpty(files['images[]'])) {
                ...
                for (let i = 0; i < images.length; i++) {
                    ...
                    const thumb = path.join('/uploads/images', `thumb-${images[i].filename}`).replace(/\\/g, '/');
                    await Promise.all([
                        sharp(images[i].path)
                            .resize(256)
                            .jpeg({
    ...
    static updateEvent = async (req, res, next) => {
        try {
            ...
            const {start_date, end_date, speaker_id, category_id, longitude, latitude, name, details, delete_image_ids} = req.body;
    ...
    static getEvents = async (req, res, next) => {
        try {
            ...
            const queryObj = {
                ...
                attributes: [
                    'id', 'name', 'details', 'start_date', 'end_date',
            ...
            if (!_.isEmpty(categoryIds)) {
                queryObj.order = [
                    [
                        Sequelize.literal(`CASE WHEN category_id IN (${categoryIds.join(',')}) THEN 0 ELSE 1 END`),
                        'ASC',
                    ],
                    ['category_id', 'ASC'],
                ]
